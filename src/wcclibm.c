...
/* instruction here infeasible */
  ...
  ix = ( q >> 1 ) + 0x3f000000;
  ix += ( m << 23 ); //FIXME: UBSan reports that this may produce undefined behavior because m may be negative
  SET_FLOAT_WORD( z, ix );
  return z;
}
...
#ifdef __STDC__
float basicmath___sinf( float x )
#else
float basicmath___sinf( x )
float x;
#endif
{
  float y[  2  ], z = 0.0;
  ...
  ix &= 0x7fffffff;
  if ( ix <= 0x3f490fd8 )
    return ( basicmath___kernel_sinf( x, z, 0 ) );
